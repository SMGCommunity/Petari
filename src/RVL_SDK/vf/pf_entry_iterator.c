            ...
            if (p_iter->sector == 0xFFFFFFFF && may_allocate != 0) {
                ...
                if (err == 0) {
                    if (p_iter->sector != 0xFFFFFFFF && is_initialize != 0xFFFFFFFF) {
                        err = VFiPFENT_FillVoidEntryToSectors(p_iter->p_vol, p_iter->sector, p_iter->p_vol->bpb.sectors_per_cluster, 0, 0, 0);
                    }
                }
            }
        }
...
static s32 VFiPFENT_ITER_GetEntry(struct PF_DIR_ENT* p_ent, struct PF_ENT_ITER* p_iter, struct PF_STR* p_pattern, u8 attr_required, u32* p_lpos,
    ...
    if (is_match == 1) {
        if ((attr & 0xF) == 0xF) {
            if (VFiPFENT_LoadLFNEntryFieldsFromBuf(p_ent, p_iter->buf, is_reverse) != 0) {
                p_ent->num_entry_LFNs = 0;
                p_ent->long_name[0] = 0;
            }
        } else {
        ...
        *p_is_found = 1;
        ...
        while (!VFiPFENT_ITER_IsAtLogicalEnd(p_iter)) {
            ...
            if (p_iter->buf[0] == 0xE5) {
                ...
                err = VFiPFENT_ITER_GetEntry(p_ent, p_iter, p_pattern, attr_required, p_lpos, 0);
                if (err == 0) {
                    p_ent->p_vol = p_iter->ffd.p_vol;
                    if (p_ppos) {
                        *p_ppos = p_iter->index;
...
static s32 VFiPFENT_ITER_DoAllocateEntry(PF_DIR_ENT* p_ent, u8 num_entries, PF_FFD* p_ffd, u32* p_prev_chain, PF_STR* p_filename, u8 attr_required,
    ...
    while (err == 0) {
        if (!is_found) {
            ...
            if (iter.buf[0] == 0 || iter.buf[0] == 0xE5) {
                if (sector_c != iter.sector) {
                    sector_a = sector_b;
                    sector_b = sector_c;
                    sector_c = iter.sector;
                }
                run_count++;
                if (run_count >= num_entries) {
                    if (p_pos) {
                        *p_pos = iter.index;
                    }
                    run_sector = iter.sector;
                    is_found = 1;
                    run_offset = iter.offset;
                }
            } else {
                run_count = 0;
                sector_b = 0xFFFFFFFF;
        ...
        if (p_filename) {
            if (is_found && iter.buf[0] == 0) {
                break;
            }
            if (iter.buf[0] != 0 && iter.buf[0] != 0xE5) {
                err = VFiPFENT_ITER_GetEntry(&wk_ent, &iter, p_filename, attr_required, 0, 0);
                if (err == 0) {
                    *p_ent = wk_ent;
    ...
    p_prev_chain[0] = sector_b;
    p_prev_chain[1] = sector_a;
    p_ent->entry_sector = run_sector;
    p_ent->entry_offset = run_offset;
    return 0;
}
...
u32 VFiPFENT_ITER_IsAtLogicalEnd(struct PF_ENT_ITER* p_iter) {
    ...
    if (p_iter->sector == 0xFFFFFFFF || p_iter->buf[0] == 0 || p_iter->index >= 999999) {
        ...
        err = VFiPFENT_ITER_RecalcEntryIterator(p_iter, 0);
        if (err != 0) {
...
s32 VFiPFENT_ITER_FindEntry(struct PF_ENT_ITER* p_iter, struct PF_DIR_ENT* p_ent, struct PF_STR* p_pattern, u8 attr_required, u32* p_is_found,
...
s32 VFiPFENT_ITER_AllocateEntry(PF_DIR_ENT* p_ent, u8 num_entries, PF_FFD* p_ffd, u32* p_prev_chain, PF_STR* p_filename, u8 attr_required,
    ...
    if (p_filename && (VFipf_vol_set.setting & 2) == 2 && VFiPFPATH_CheckExtShortNameSignature(p_filename)) {
        return 1;
    }
    return VFiPFENT_ITER_DoAllocateEntry(p_ent, num_entries, p_ffd, p_prev_chain, p_filename, attr_required, p_pos);
}
...
s32 VFiPFENT_ITER_GetEntryOfPath(struct PF_ENT_ITER* p_iter, struct PF_DIR_ENT* p_ent, struct PF_VOLUME* p_vol, struct PF_STR* p_path,
    ...
    return VFiPFENT_ITER_DoGetEntryOfPath(p_iter, p_ent, p_vol, p_path, 0, is_parent);
}
