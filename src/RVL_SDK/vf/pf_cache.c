...
int VFiPFCACHE_InitPageList(PF_VOLUME* p_vol, PF_CACHE_PAGE** pp_head, PF_CACHE_PAGE* pages, u8 (*buffers)[512], u16 num, u32 size, u32 is_fat) {
    ...
    *pp_head = pages;
    ...
    if (num / bps_per_buf == 1) {
        ...
        pages[num - num % bps_per_buf - bps_per_buf].stat &= ~3u;
        if (is_fat)
            pages[num - num % bps_per_buf - bps_per_buf].stat |= 4u;
        else
            pages[num - num % bps_per_buf - bps_per_buf].stat &= ~4u;
        return 0;
    }
    ...
    if (*pp_page) {
        ...
        *p_is_hit = 0;
        if (!VFiPFCACHE_SearchForFreePage(*pp_head, pp_page)) {
            ...
            if (*pp_page && ((*pp_page)->stat & 2) != 0 && (*pp_page)->sector != -1) {
                do_flush = 1;
            }
        ...
        if (p_page && p_page->sector != -1) {
            if (p_page->sector <= sector && p_page->sector + p_page->size >= sector + num_sector) {
                ...
                *p_num_success += num_rest_sector;
                ...
                p_ebuf = (((num_sector - 1) << p_vol->bpb.log2_bytes_per_sector) + p_sbuf);
                ...
                *p_num_success += p_page->size - (p_page->sector + p_page->size - (sector + num_sector));
                p_ebuf =
                    &p_page->buffer[(p_page->size - (p_page->sector + p_page->size - (sector + num_sector)) - 1) << p_vol->bpb.log2_bytes_per_sector];
...
s32 VFiPFCACHE_DoFlushCache(PF_VOLUME* p_vol, PF_CACHE_PAGE* p_head) {
    ...
    if (do_flush) {
        set_device_err = first_err == 0;
        if (dCommon_FlushFromVol(p_vol, set_device_err) && set_device_err == 1) {
            return 17;
        }
    }
    return first_err;
...
s32 VFiPFCACHE_FlushDataCacheSpecific(PF_VOLUME* p_vol, void* signature) {
    ...
    if ((p_vol->cache.mode & 1) == 0) {
        ...
        if (!p_head->signature || p_head->signature == signature) {
            if (p_head && (p_head->stat & 2) != 0 && p_head->sector != -1)
                do_flush = 1;
