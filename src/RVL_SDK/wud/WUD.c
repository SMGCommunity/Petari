...
{
    OSSwitchFiberEx((u32)alarm, (u32)context, 0, 0,
                    (u32)EnableStackHandler,
                    (u32)(__WUDHandlerStack + sizeof(__WUDHandlerStack)));
}
    ...
    switch(event)
    {
        case 0:
            ...
            WUD_DEBUGPrint("host : %02x:%02x:%02x:%02x:%02x:%02x\n", 
                p_wcb->hostAddr[0], p_wcb->hostAddr[1], p_wcb->hostAddr[2], 
                p_wcb->hostAddr[3], p_wcb->hostAddr[4], p_wcb->hostAddr[5]
            );
        ...
        case 3:
            ...
            WUD_DEBUGPrint("  key  : %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
                p_authcmpl->key[0],  p_authcmpl->key[1],  p_authcmpl->key[2],  p_authcmpl->key[3], 
                p_authcmpl->key[4],  p_authcmpl->key[5],  p_authcmpl->key[6],  p_authcmpl->key[7], 
                p_authcmpl->key[8],  p_authcmpl->key[9],  p_authcmpl->key[10], p_authcmpl->key[11], 
                p_authcmpl->key[12], p_authcmpl->key[13], p_authcmpl->key[14], p_authcmpl->key[15] 
            );
