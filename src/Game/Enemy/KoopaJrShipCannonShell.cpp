...
void KoopaJrShipCannonShell::init(const JMapInfoIter& rIter) {
    ...
    initSound(4, false);
    initHitSensor(2);
    MR::addHitSensorEnemy(this, "body", 8, 75.0f, TVec3f(0.0f, 0.0f, 0.0f));
    MR::addHitSensorEnemyAttack(this, "attack", 8, 60.0f * getBaseScale(), TVec3f(0.0f, 0.0f, 0.0f));
    ...
    MR::initStarPointerTarget(this, 150.0f, TVec3f());
...
void KoopaJrShipCannonShell::exeFly() {
    ...
    if (MR::isGreaterStep(this, 10) && MR::isBinded(this)) {
        explosion();
        return;
    }
...
void KoopaJrShipCannonShell::exeDown() {
    ...
    JMathInlineVEC::PSVECAdd(mVelocity, mGravity.scaleInline(1.0f), mVelocity);
    if (MR::reboundVelocityFromCollision(this, 0.55f, 2.0f, 1.0f))
        MR::deleteEffect(this, "LocusSmoke");

    mVelocity.scale(MR::isBindedGround(this) ? 0.94999999f : 0.99f);
...
void KoopaJrShipCannonShell::exeFreeze() {
    ...
    f32 scale = 5.0f * JMath::sSinCosTable.cosLap(MR::repeatDegree(_B4 * 0.75f)) * static_cast<f32>(20 - getNerveStep()) / 20.0f;
    TVec3f vec14;
