...
LavaStrangeRock::LavaStrangeRock(const char* pName) : LiveActor(pName),
    rockType(3),
    _90(0),
    mLodCtrlPlanet(NULL)
{}
...
void LavaStrangeRock::initModelAndClipping(const JMapInfoIter& iter) {
    ...
    switch(rockType) {
    case 0:
        initModelManagerWithAnm("LavaStrangeRock", NULL, false);
        MR::setClippingTypeSphere(this, (mScale.y * 870.0f));
        break;
    case 1:
        initModelManagerWithAnm("LavaStrangeRockL", NULL, false);
        MR::setClippingTypeSphere(this, (mScale.y * 1300.0f));   
        break;
    case 2:
        initModelManagerWithAnm("LavaStrangeRockTable", NULL, false);
...
void LavaStrangeRock::exeWait() {
    ...
    if(_90 == 1) {
        if(!(MR::isPlayerInRush())) return;
        TVec3f upPlusPosition = mPosition.operator+(upVec2);
        TVec3f *pCenterPos2 = MR::getPlayerCenterPos();
        if ((MR::checkHitSegmentSphere(*pCenterPos2, mPosition, upPlusPosition, v1, 0)) != 1) return;
    ...
    if(!(MR::checkHitSegmentSphere(*MR::getPlayerCenterPos(), mPosition, upPlusPosition2, v1, &checkHitSegSphereRes))) return;
