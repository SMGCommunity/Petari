...
def makeElf():
    ...
    linker_path = pathlib.Path(f"../../Compilers/{default_compiler_path}/mwldeppc.exe ")
    linker_flags = f"-lcf ldscript.lcf -fp hard -proc gekko -map RVLFaceLib.map -o RVLFaceLib.elf {fileList}"
    if subprocess.call(f"{linker_path} {linker_flags}", shell=True) == 1:
...
def main(compile_non_matching, use_ninja, clean_ninja, link):
    ...
    msl_c_path =        pathlib.Path("../MSL_C/include")

    include += f"-i {rvl_sdk_path} -I- -i {trk_path} -I- -i {runtime_path} -I- -i {msl_c_path} "
    flags += include
