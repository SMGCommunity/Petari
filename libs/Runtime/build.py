...
def makeLibArchive():
    ...
    linker_flags = f"-nodefaults -xm l -o lib/Runtime.a {fileList}"
...
def main(compile_non_matching, use_ninja, clean_ninja, link):
    ...
    flags = "-c -nodefaults -nostdlib -proc gekko -use_lmw_stmw on -enum int -fp hard -rtti off -Cpp_exceptions on -O4 -inline auto "
    ...
    includes += f"-i {rvl_path} -i {trk_path} -I- -i {msl_path} -I- -i {facelib_path} "
    flags += includes
