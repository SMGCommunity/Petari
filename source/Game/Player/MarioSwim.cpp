...
inline const Constants* getConstants(const MarioActor *actor) {
    MarioConst *constSelector = actor->mConst;
...
bool MarioSwim::update() { // 1f = 805cbc80
    ...
    if(!_19 || (_20 && checkTrgA())) {
        if(_7A || _7C) _9C = 0;
            ...
            if(!checkLvlA()) {
                ...
                if((!_1B0 || _8A || getPlayer()->mMovementStates._18 || getPlayer()->mMovementStates._2D) && checkTrgA()) {
                    _28 = tmp;
                }
            }
            else {
            ...
            if(_1B0) {
                if(_24 > 6) {
                    f32 res = MR::clamp(_54 / getConstants(mActor)->_520, 0f, 1f);
                    //tmp = 0;
    ...
    if(_32 && !--_32 == 0) {
        r1e = true;
        _5C = getConstants(mActor)->_54C;
        _2C = getConstants(mActor)->_550;
        _2E = getConstants(mActor)->_552;
        _54 = getConstants(mActor)->_558;
        _38 = 0x14;
    }
    f32 fr1f = 1f - MR::clamp(_54 / getConstants(mActor)->_4B4, 0f, 1f);
    ...
    if(_38) _38--;
        ...
        if(_19 && !_32) {
            if(!_18) {
                if(_8A) {
                    ...
                    _4C = _4C * (1f - getConstants(mActor)->_4F4) + getStickY() * (1f - getConstants(mActor)->_4F4);
                    f32 fr1c = 1f;
                    _50 = 2.5f * getStickX() * (fr1c - getConstants(mActor)->_4F8);
                    ...
                    playEffect("水面Ｚ沈降");
                    playSound("水面潜り", -1);
                    _32 = getConstants(mActor)->_554;
                }
                else if(checkLvlZ() == 0) res = 0f;
                else {
                    _4C = _4C * getConstants(mActor)->_4EC + getStickP() * (1f - getConstants(mActor)->_4EC);
                }
            }
            _50 *= getConstants(mActor)->_4F0;
        }
        else {
            ...
            if(_8A) {
                _4C = _4C * getConstants(mActor)->_4F4 + getStickY() * (1f - getConstants(mActor)->_4F4);
                 _50 = _50 * getConstants(mActor)->_4F8 + getStickX() * (1f - getConstants(mActor)->_4F8);
            }
            else {
                _4C = _4C * getConstants(mActor)->_4EC + getStickY() * (1f - getConstants(mActor)->_4EC);
                 _50 = _50 * getConstants(mActor)->_4F0 + getStickX() * (1f - getConstants(mActor)->_4F0);
            }
        }
        if(_20 && _3C && _2C) {
            _5C += res * _4C * getConstants(mActor)->_4E0; // Sequentially after the other setting of res
        }
        ...
        if(_8A) {
            switch(_88) {
                case 0:
                    fr1e *= getConstants(mActor)->_52C;
                    break;
                default:
                    fr1e *= getConstants(mActor)->_530;
                    break;
            }
        }
        if(_20) {
        ...
        if(_32) {
            fr1e = 0.15f;
            fr1d = getConstants(mActor)->_54C;
        }
        bool r1b = true;
        f32 prod = fr1d * fr1e;
        _5C = _5C * (1f - prod) + prod;
        if(_3C && _AE && _32 && getStickY() > 0f) {
            fr1d = 3.14159274101f / getConstants(mActor)->_5B8 * (getStickY() - 0.523598790169f) ;
            if(_5C < fr1d) {
                _5C = _5C * (1f - getConstants(mActor)->_5BC) + fr1d * (1f - getConstants(mActor)->_5BC);
            }
            else _5C = _5C * (1f - fr1d * fr1e) + fr1d * fr1e;
            if(!(_7A || _7C) && !_8A) changeAnimation("水泳ターン下", (const char *)nullptr);
            r1b = false;
        }
        if(r1b) stopAnimation("水泳ターン下", (const char *)nullptr);
        if(_32) {
            _5C = MR::clamp(_5C, 0.523598790169f, getConstants(mActor)->_54C);
        }
    ...
    if((_20 || (_19 && isStickOn())) && _8A) {
        ...
        if(_8A) tmp *= 3f;
        f32 res = MR::clamp(tmp * getConstants(mActor)->_518, 0f, 1f);
        ...
        f32 tmp = fr1f - getConstants(mActor)->_4E8;
        MR::rotAxisVecRad(_60, -_6C, &_60, tmp * _50 * getConstants(mActor)->_4E4);
    }
    ...
    f32 fr1d = fr2 + (1f - fr2) * getConstants(mActor)->_514;
    ...
    if(!_19 && !_2C && !_8A) procBuoyancy();
        ...
        if(obj) {
            ...
            if(followMtx) {
                ...
                if (!(
                    JGeometry::TUtil<f32>::epsilonEquals(stack_F8.x, _154.x, 0.000003814697265625f)
                    && JGeometry::TUtil<f32>::epsilonEquals(stack_F8.y, _154.y, 0.000003814697265625f)
                    && JGeometry::TUtil<f32>::epsilonEquals(stack_F8.z, _154.y, 0.000003814697265625f)
                )) {
    ...
    if(_8A) {
        addVelocity(_184, getConstants(mActor)->_534);
    }
    ...
    if(!MR::normalizeOrZero(&stack_EC)) {
        if(MR::diffAngleAbs(stack_EC, _60) < 1.57079637051f) {
            ...
            MR::vecBlendSphere(_60, -stack_EC, &_60, fr1dv2);
        }
    }
    TVec3f stack_E0;
    ...
    if(!_8A && !_19 && !MR::isNearZero(_190, 0.001f)) {
        ...
        if(angleDiff > 1.04719758034f) {
            ...
            PSMTXMultVecSR(stack_1A0, _6C.toCVec(), _6C.toVec());
        }
    }
    decideAnimation();
    decideEffect(false);
