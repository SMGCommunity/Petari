...
void MarioActor::init2(const TVec3f &a, const TVec3f &b, long initialAnimation)
{
    ...
    _27C.zero();
    _288.zero();
    _498 = new FixedPosition(this, "HandR", TVec3f(0.0f, 0.0f, 0.0f), TVec3f(0f, 0f, 0f));
...
void MarioActor::changeGameOverAnimation()
{
    ...
    if (mMario->isAnimationRun("�d�C�_���[�W�I��")) {
        animation = 1;
    }
    if (mMario->isAnimationRun("���_���[�W")) {
        animation = 2;
    }
    if (mMario->isAnimationRun("�t�@�C�A�_���X")) {
        animation = 2;
    }
    if (mMario->isAnimationRun("���_���[�W")) {
        animation = 3;
    ...
    if (mMario->isAnimationRun("���_���[�W���n")) {
        animation = 3;
    }
    if (mMario->isAnimationRun("����_���[�W")) {
        animation = 4;
    ...
    if (mMario->isAnimationRun("����")) {
        animation = 5;
    }
    if (mMario->isAnimationRun("�󒆂ӂ�΂�")) {
        animation = 5;
    }
    if (mMario->isAnimationRun("�Ԃ�")) {
        animation = 6;
    }
    if (mMario->isAnimationRun("�Ԃꕜ�A")) {
        animation = 6;
    }
    if (!mMario->isAnimationRun("�X��") && !mMario->isStatusActive(0xd))
        ;
    else {
        animation = -1;
    }
    ...
    switch (animation) {
    case 0:
        mMario->changeAnimationNonStop("����_�E��");
        break;
    case 1:
        mMario->changeAnimationNonStop("���d�_�E��");
        break;
    case 2:
        mMario->changeAnimationNonStop("���_�E��");
        break;
    case 3:
        ...
        break;
    case 4:
        mMario->changeAnimationNonStop("�낹�_�E��");
        break;
    case 5:
        if (mMario->getMovementStates()._1) {
            ...
            mMario->changeAnimationNonStop("�ޗ��_�E��");
        }
        break;
    case 6:
        mMario->changeAnimationNonStop("�Ԃ�_�E��");
        break;
    case 7:
        mMario->changeAnimationNonStop("���j�_�E��");
        break;
    case 8:
        mMario->changeAnimationNonStop("���܂�_�E��");
        break;
    case 9:
        mMario->changeAnimationNonStop("���[�X����");
        break;
    case 10:
        mMario->changeAnimationNonStop("�������[�X����");
        break;
...
void MarioActor::movement()
{
    ...
    if (MR::isOppositeDirection(_288, stack_11C, 0.01f)) {
        ...
        if (!MR::isNearZero(mag_288, 0.001f) && !MR::isNearZero(magStack_11C, 0.001f) && MR::isNearZero(mag_288 - magStack_11C, 1f)) {
            mPosition -= _288 % 0.5f;
        }
    }
    if (PSVECMag(stack_128.toCVec()) > 0.1f) {
        if (!(getMovementStates()._A)) {
            if (!MR::isNearZero(mVelocity, 0.001f)) {
                ...
                if (PSVECMag(stack_128.toCVec()) > 2f * (diffMag + vMag)) {
                    mMario->stopWalk();
                }
            }
        }
        if (getMovementStates()._23 && PSVECMag(mVelocity.toCVec()) < PSVECMag(stack_134.toCVec())) {
            ...
            if (PSVECMag(mVelocity.toCVec()) > 20f && elementA < elementB * 0.5f) {
                if (mMario->isAnimationRun("�₷�ׂ艺���������ނ�")) {
                    mMario->push(mMario->_208 % 5f);
                }
                else if (mMario->isAnimationRun("�₷�ׂ��������Ԃ�")) {
                    mMario->push(mMario->_208 % -5f);
                }
        ...
        if (getMovementStates()._0 && !_9F1) {
            ...
            for (u32 i = 0; i < stop; i++) {
                ...
                if (mMario->_368.dot(*plane->getNormal(0)) > 0f) {
                    ...
                    else if (plane->getNormal(0)->dot(stack_134) < 0f) {
                        eject = true;
                    }
                    else {
                        eject = false;
...
void MarioActor::control()
{
    if (mSuperKinokoCollected) {
        if (MR::tryStartDemoWithoutCinemaFrame(this, "�}���I�X�[�p�[��")) {    // 6-up camera
            mSuperKinokoCollected = false;
            changeMaxLife(6);
            MR::stopAnimFrame(this);
            MR::requestPowerupHPMeter();
            mMario->startPadVib("�}���I[�ϐg]");
            MR::startSystemSE("SE_SY_SUPER_KINOKO_GET", -1, -1);
            _3DA = 0x5a;
        }
    }
    else if (mPowerupCollected) {
        if (MR::tryStartDemoWithoutCinemaFrame(this, "�}���I�ϐg")) {
            mPowerupCollected = false;
            mTransforming = true;
            if (_3D4 == 6) {
...
void MarioActor::updateBehavior()
{
    ...
    updateEffect();
    if (_B94 && !--_B94) {
        mMario->stopAnimationUpper("�n���}�[������]��", nullptr);
        mMario->stopAnimation("�n���}�[������]��", (const char *)nullptr);
    }
    updatePunching();
...
void MarioActor::updatePunching()
{
    if (_944) {
        ...
        if (canSpinring && !mMario->isSwimming() && !_944 && selectAction("�X�s���񕜃G�t�F�N�g") == true) {
            playEffect("�X�s�������O");
        }
    ...
    if (mMario->isAnimationRun("�n���}�[���������[�X") && mMario->getMovementStates()._1 && !_38C && !mMario->_420 && mMario->Mario::isStickOn()) {
        ...
        if (selectWaterInOutRush(_924)) {
            ...
            if ((int)mMario->mSwim->_144 != initial) {
                if (mMario->mSwim->_144 <= 1 && (u32)initial - 2 <= 1) {
                    playEffectRT("���ʃW�����v����", mMario->mSwim->_160, mMario->mSwim->_16C);
                    emitEffectWaterColumn(mMario->mSwim->_160, mMario->mSwim->_16C);
                }
                else if ((u32)initial <= 1 && mMario->mSwim->_144 - 2 <= 1) {
                    playEffectRT("���ʃW�����v����", -mMario->_328, mMario->mSwim->_16C);
                    emitEffectWaterColumn(mMario->mSwim->_160, mMario->mSwim->_16C);
                }
                if (initial == 2) {
                    _384 = 8;
...
void MarioActor::updateSwingTimer()
{
    if (_934) {
        ...
        if (_946) {
            if (--_946 == 0x18) {
                selectAction("�X�s���񕜃G�t�F�N�g");
                stopEffectForce("�X�s�������O");
            }
            if (_946 == 0xd) {
                _94C = 0x13;
            }
...
void MarioActor::updateSwingAction()
{
    ...
    u8 action = selectAction("�X�s���A�^�b�N");
    switch (action) {
    case 1:
        ...
        if (!mMario->mMovementStates._F && isJumping() && !mMario->isDamaging() && !mMario->mMovementStates._2B) {
        ...
        else if (!getMovementStates()._F && !mMario->isAnimationRun("�n��Ђ˂�")) {
            const char *pLastAnimationName = mMarioAnim->_10->getCurrentAnimationName();
            if (_3D4 == 4) {
                if (!mMario->isAnimationRun("�n�`�X�s��")) {
            ...
            if (pLastAnimationName != mMarioAnim->_10->getCurrentAnimationName()) {
                mMario->playSound("�p���`���؂�", -1);
            }
        }
        if (_3D4 == 4) {
            if (isJumping()) {
                if (!mMario->isAnimationRun("�n�`�X�s����")) {
                    mMario->playSound("���X�s��", -1);
                    mMario->playSound("�X�s���W�����v", -1);
                }
                mMario->changeAnimation("�n�`�X�s����", (const char *)nullptr);
            }
            else if (getMovementStates()._A || _9F1) {
                mMario->changeAnimation("�T�}�[�\���g", (const char *)nullptr);
            }
            else {
                mMario->changeAnimation("�n�`�X�s��", (const char *)nullptr);
            }
    ...
    case 5:
        ...
        if (isJumping()) {
            ...
            if (getMovementStates()._A || _9F1) {
                mMario->changeAnimation("�T�}�[�\���g", (const char *)nullptr);    // Summersault
            }
            else {
                mMario->changeAnimation("�n�`�X�s��", (const char *)nullptr);
            }
...
static const char *sMiddleWaterLifeReduction = "�������C�t����";
...
void MarioActor::incLife(unsigned long amt)
{
    if (isEnableNerveChange() && !_3E4) {
        ...
        if (health == 1 && mMarioAnim->isAnimationStop()) {
            mMarioAnim->_C->changeTrackAnimation(3, "�m�[�}���E�G�C�g");
...
bool MarioActor::doPressing()
{
    ...
    case 0:
        if (!mMario->checkVerticalPress(false)) {
            ...
            if (!_390) {
                mMario->changeAnimation("�Ԃ����", (const char *)nullptr);
                _F44 = true;
            }
        }
        break;
